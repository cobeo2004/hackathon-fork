...
export function SolutionSection() {
  ...
  return (
    <div>
      <SectionLead step={2} eyebrow="The solution" title="Predict the failure, then plan the smartest collection" subtitle="Inverter health data becomes a risk score, a failure window, a collection job, and a verifiable record — automatically." />
      <Card className="p-5"><Pipeline /></Card>
      <div className="mt-5 grid gap-5 lg:grid-cols-3">
        ...
        <Card className="flex flex-col p-5">
          <div className="mb-4 font-mono text-[11px] uppercase tracking-[0.14em] text-muted">Digital passport</div>
          <div className="flex-1"><PassportPanel events={passport} /></div>
        </Card>
      </div>
      <AlgorithmStrip />
      <DataNote real="solar lifespan assumptions (25-yr panels) & the end-of-life cohort from CER install-age data" illustrative="this inverter's identity, live telemetry, risk score & passport events (no public live-IoT feed exists)" source="rule-based engine on synthetic telemetry; lifespans per manufacturer / Sustainability Victoria guidance" />
    </div>
  );
}

const ALGO_STEPS = [
  { tag: "Predict", name: "Rule-based risk score", detail: "Weighted signals → breaking-risk band", formula: "0.30·temp + 0.25·THD + 0.20·eff + 0.15·volt + 0.10·age" },
  { tag: "Filter", name: "Greedy knapsack", detail: "Pack stops under truck mass capacity", formula: "Σ mass ≤ 2,500 kg, priority-first" },
  { tag: "Optimize", name: "Exact brute-force TSP", detail: "Shortest depot → stops → centre", formula: "min over all permutations (greedy NN fallback)" },
  { tag: "Measure", name: "Haversine distance", detail: "Great-circle km between sites", formula: "2r·asin(√hav(Δφ,Δλ))" },
];
function AlgorithmStrip() {
  return (
    <Card className="mt-5 p-5">
      <div className="mb-4 flex items-center gap-3">
        <span className="font-mono text-[11px] font-semibold uppercase tracking-[0.18em] text-solar">Under the hood</span>
        <span className="font-mono text-[11px] uppercase tracking-wide text-muted">the algorithms that turn data into a plan</span>
      </div>
